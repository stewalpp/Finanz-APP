/* ============================================================
   Unsere Finanzen — js/icons.js
   Lucide Icons (https://lucide.dev) — ISC-Lizenz.
   Als Inline-SVG gebuendelt, damit die PWA offline funktioniert
   (kein CDN, kein Icon-Font). App.icon(name, size) erzeugt ein
   <svg>-Element, App.catIcon(catKey, size) den runden farbigen
   Kategorie-Kreis mit dem passenden Icon.
   Classic script, loaded right after core.js. No modules.
   ============================================================ */
(function () {
  'use strict';
  ...
  /* Inneres SVG-Markup je Icon (viewBox 0 0 24 24, stroke-basiert). */
  App.ICONS = {
  ...
  // <svg>-Element fuer ein Icon; unbekannte Namen fallen auf 'package' zurueck.
  App.icon = function (name, size) {
    ...
    svg.innerHTML = App.ICONS[name] || App.ICONS['package'];
    return svg;
  };

  // Runder Kategorie-Kreis (Vollton-Farbe) mit weissem Icon darin.
  App.catIcon = function (catKey, size) {
    var c = App.cat(catKey);
    var div = App.el('div', 'cat-icon');
    div.style.background = c.color;
    if (size) {
      div.style.width = size + 'px';
      div.style.height = size + 'px';
    }
    div.appendChild(App.icon(c.icon, Math.round((size || 36) * 0.55)));
    return div;
  };
